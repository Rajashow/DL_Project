    ...
    def __init__(self, directory, is_train):
        ...
        for filename in os.listdir(directory):
            if filename.endswith(".npy"): 
                ...
                if is_train:
                    self.num_images += int(len(data)*0.016)
                    self.fsize[i] = int(len(data)*0.016)
                    self.fimgsize[i] = self.num_images
                else:
                    self.num_images += int(len(data)*0.004)
                    self.fsize[i] = int(len(data)*0.004)
                    self.fimgsize[i] = self.num_images
                i += 1
        ...
        if self.is_train:
            ...
            for i in range(0, self.num_class):
                if ind - self.fimgsize[i] < 0 and i == 0:
                    ...
                    ind_offset = int(len(np_dat)*0.016)
