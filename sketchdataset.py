    ...
    def __init__(self, directory, is_train):
        ...
        for filename in os.listdir(directory):
            if filename.endswith(".npy"): 
                ...
                if is_train:
                    self.num_images += int(len(data)*0.008)
                    self.fsize[i] = int(len(data)*0.008)
                    self.fimgsize[i] = self.num_images
                else:
                    self.num_images += int(len(data)*0.002)
                    self.fsize[i] = int(len(data)*0.002)
                    self.fimgsize[i] = self.num_images
                i += 1
        ...
        if self.is_train:
            for i in range(0, self.num_class):
                if ind - self.fimgsize[i] < 0 and i == 0:
                    img_dat = np.load(self.fnames[0])[ind].reshape(1, 28, 28).astype(np.float32)
                    return torch.from_numpy(img_dat), i
                elif ind - self.fimgsize[i] < 0:
                    img_dat = np.load(self.fnames[i])[ind - self.fimgsize[i-1]].reshape(1, 28, 28).astype(np.float32)
                    return torch.from_numpy(img_dat), i
        else:
            for i in range(0, self.num_class):
                if ind - self.fimgsize[i] < 0 and i == 0:
                    np_dat = np.load(self.fnames[0])
                    ind_offset = int(len(np_dat)*0.008)
